...
def train_unified(df: pd.DataFrame) -> dict:
    ...
    from model.prob_resid import add_magnus, add_shape_features, fit_cutter_router, assign_family, bucket_values, train_split_model, predict_group_rv, fit_spin_offset, SHAPE_FEATS
    add_magnus(df)
    fit_spin_offset(df)
    ...
    if _hist is not None:
        ...
        if len(hi):
            ...
            eh = predict_group_rv(df.loc[hi], ens)
