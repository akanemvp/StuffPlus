...
_SCORED_COLS = [
    ...
    "stuff_plus_rhb", "stuff_plus_lhb", "estimated_woba_using_speedangle",
]

try:
    from model.prob_resid import SHAPE_FEATS as _MODEL_FEATS
    _needed = list(_MODEL_FEATS) + \
...
def _compute_zone(plate_x, plate_z, sz_top, sz_bot) -> "int | None":
    ...
    if -0.8333 <= px <= 0.8333 and sb <= pz <= st:
        x_frac = (px + 0.8333) / 1.6666
        z_frac = (pz - sb) / max(st - sb, 0.01)
...
def _refresh_spring() -> None:
    ...
    try:
        ...
        all_spring_pks = _get_spring_game_pks()
        csv_game_pks = (
...
def _refresh_live() -> None:
    ...
    try:
        ...
        all_live_pks = _get_live_game_pks()
        csv_game_pks = (
            set(csv_df["game_pk"].dropna().astype(int))
            if not csv_df.empty and "game_pk" in csv_df.columns else set()
        )
        ...
        if "player_name" in to_add.columns:
            to_add["player_name"] = to_add["player_name"].apply(_gf_name)
            to_add = _apply_canonical_names(to_add)
        conn_raw = sqlite3.connect(DB_PATH)
        try:
...
def _crop_trailing_bg(png_path: str, pad: int = 28) -> None:
    try:
        ...
        trail = a[-3, -3]
        content = (np.abs(a - trail).sum(axis=2) > 24)
    ...
    try:
        subprocess.run(
            [_CHROME_BIN, "--headless", "--disable-gpu", "--hide-scrollbars", "--no-sandbox",
             ...
             "--window-size=1320,3200", "--virtual-time-budget=13000", url],
...
_boxscore_ts: dict[int, float] = {}
_final_games: set[int] = set()
_BOXSCORE_TTL = 90.0
...
def _compute_game_stats(gdf: pd.DataFrame) -> dict:
    ...
    if "game_pk" in gdf.columns and "pitcher" in gdf.columns:
        ...
        if not game_pk_val.empty and not pitcher_val.empty:
            ...
            er_map = _get_boxscore_earned_runs(gp_i)
...
def api_player_games(player_name_season: str):
    ...
    try:
        ...
        if "game_pk" not in have or "game_date" not in have:
            conn.close()
            return jsonify([])
        cols = [c for c in ["game_pk", "game_date", "inning_topbot", "home_team", "away_team"] if c in have]
        df = pd.read_sql_query(
            f"SELECT {', '.join(cols)} FROM {table} WHERE player_name = ?",
    ...
    for (game_pk, game_date), gdf in df.groupby(["game_pk", "game_date"], sort=False):
        ...
        if all(c in gdf.columns for c in ["inning_topbot", "home_team", "away_team"]):
            mode_tb = gdf["inning_topbot"].dropna().mode()
            topbot = mode_tb.iloc[0] if len(mode_tb) > 0 else "Bot"
            home = str(gdf["home_team"].dropna().mode().iloc[0]) if gdf["home_team"].notna().any() else ""
            away = str(gdf["away_team"].dropna().mode().iloc[0]) if gdf["away_team"].notna().any() else ""
            pitcher_team, opp_team = (home, away) if topbot == "Top" else (away, home)
        games.append({"game_pk": int(game_pk), "game_date": str(game_date),
                      "opp_team": opp_team, "pitcher_team": pitcher_team})
...
def api_game_detail():
    ...
    for _, row in summary.iterrows():
        ...
        grade, color = stuff_grade(sp_r)
        grade_l, color_l = stuff_grade(sp_l)
        def _sl(key):
...
def admin_retrain():
    ...
    def _run():
        try:
            ...
            from features.engineering import engineer_features
            from model.train import train_platoon
...
def api_player_pitches_raw(player_name_season: str):
    ...
    try:
        ...
        if scored_tbl in all_tables:
            table = scored_tbl
            sp_r_expr, sp_l_expr = "stuff_plus_rhb", "stuff_plus_lhb"
        elif raw_tbl in all_tables:
            table = raw_tbl
            sp_r_expr = sp_l_expr = "NULL"
        ...
        if sp_r_expr == "stuff_plus_rhb" and "stuff_plus_rhb" not in col_names:
            sp_r_expr = sp_l_expr = "NULL"
        ...
        df = pd.read_sql_query(
            ...
            f"  ({ivb_expr}) AS ivb, "
            f"  ({hb_expr}) AS hb, "
            f"  pitch_type AS pt, ({sp_r_expr}) AS sp_rhb, ({sp_l_expr}) AS sp_lhb, "
...
def api_reclassify():
    ...
    orig_r: dict[str, float] = {}
    orig_l: dict[str, float] = {}
    try:
        conn = sqlite3.connect(DB_PATH)
        if game_pk is not None:
            scored = pd.read_sql_query(
                f"SELECT pitch_type, stuff_plus_rhb, stuff_plus_lhb FROM pitches_{season}_scored "
                f"WHERE player_name = ? AND CAST(game_pk AS INTEGER) = ?",
                conn, params=(db_name, int(game_pk))
            )
        else:
            scored = pd.read_sql_query(
                f"SELECT pitch_type, stuff_plus_rhb, stuff_plus_lhb FROM pitches_{season}_scored WHERE player_name = ?",
                conn, params=(db_name,)
            )
        conn.close()
        for pt, grp in scored.groupby("pitch_type"):
            vr = grp["stuff_plus_rhb"].dropna(); vl = grp["stuff_plus_lhb"].dropna()
            if not vr.empty:
                orig_r[pt] = round(float(vr.mean()), 1)
            if not vl.empty:
                orig_l[pt] = round(float(vl.mean()), 1)
    except Exception:
        pass
    ...
    for pt, grp in df_scored.groupby("pitch_type"):
        ...
        results.append({
            "pitch_type":         pt,
            "stuff_plus_rhb_new": round(float(valid["stuff_plus_rhb"].mean()), 1),
            "stuff_plus_lhb_new": round(float(grp["stuff_plus_lhb"].dropna().mean()), 1) if grp["stuff_plus_lhb"].notna().any() else None,
            "stuff_plus_rhb_old": orig_r.get(pt),
            "stuff_plus_lhb_old": orig_l.get(pt),
            "n":              int(len(grp)),
...
def _ensure_scored_indexes() -> None:
    try:
        conn = sqlite3.connect(DB_PATH, timeout=120)
        conn.execute("PRAGMA busy_timeout=120000")
        for (t,) in conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'pitches_%_scored'"
        ).fetchall():
            cols = {r[1] for r in conn.execute(f"PRAGMA table_info([{t}])").fetchall()}
            if "player_name" in cols:
                conn.execute(f"CREATE INDEX IF NOT EXISTS ix_{t}_player ON [{t}](player_name)")
            if {"game_pk", "at_bat_number", "pitch_number"}.issubset(cols):
                conn.execute(f"CREATE INDEX IF NOT EXISTS ix_{t}_pkey ON [{t}](game_pk, at_bat_number, pitch_number)")
        conn.commit()
        conn.close()
        logger.info("scored-table indexes ensured")
    except Exception as exc:
        logger.warning(f"_ensure_scored_indexes failed: {exc}")


if __name__ == "__main__":
    import os
    _ensure_scored_indexes()
